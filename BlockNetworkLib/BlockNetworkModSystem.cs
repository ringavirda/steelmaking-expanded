...
public class BlockNetworkModSystem : ModSystem
{
  ...
  {
    if (
      neighbourBlock is not BlockNetworkNode neighbourNode
      || neighbourNode.NetworkType != sourceNode.NetworkType
      || !neighbourNode.HasConnectorAt(facing.Opposite)
    )
      return false;
  ...
  public static bool IsCompatibleNetworkBlock(Block neighbour, string id) =>
    neighbour is BlockNetworkNode netBlock && netBlock.NetworkType == id;
