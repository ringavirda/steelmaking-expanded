...
/// <c>smex:cowperstove-intake-s</c>), and the heat sink followed when it was rebuilt in
/// refractory brick rather than iron sheet; adding the group changed those codes, so old
/// placements load as missing-block placeholders. Each is rewritten to the tier3 variant of the
/// same base and orientation. (The pipe passthrough/outlet that also gained the group have since
/// moved to the ppex mod; their migration lives in <c>PipeMigration</c> there.)
/// </summary>
public class BrickVariantMigration : IBlockCodeMigration {
  ...
  [
    ...
    new("cowperstoveheatsink", "tier3", ["north", "south", "west", "east"]),
  ];
